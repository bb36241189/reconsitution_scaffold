...
app.controller("hostController",function($scope,$http,SAlert,$compile,PermitStatus,PermitStatus){
...
function render_host_table(data) {
    ...
    if (data.length) {
        for (var i = 0; i < data.length; i++) {
            ...
            //传感器处理
            var hardware = '<td title=show_hardware index='+i+'></td>';
            var hardwareCompile = '<a href="javaScript:void(0);" ng-click="click_show_hardware(\''+data[i].id+'\');">查看</a>';
            if(eventNum == -1){
                hardware = '<td title=show_hardware>--</a></td>';
            }else{
                var compileFn = $compile(hardwareCompile);
            ...
            $('td[title="show_hardware"][index="'+i+'"]').append($dom);
        }
...
function register_sync_host(){
    $("#submit_sync_host").click(function(){
        ...
        $.ajax({
            type: "get",
            url: project_url + "/hosts-discovery",
            headers: {
                "RC-Token": $.cookie("token_id")
            },
            success: function () {
