...
app.controller('DepartmentController',['$scope','$http','$q','SAlert','DepartmentService','PermitStatus',function ($scope,$http,$q,SAlert,DepartmentService,PermitStatus) {
    $(function (){
        set_navigator();
    ...
    function init_department_table(search){
        $.ajax({
            type: "GET",
            url:project_url +  "/departments"+"?name="+search,
            async: true,
            dataType: "json",
            headers:{
            ...
            success: function(data){
                ...
                department_table_pagination(data.departments);
            },
            error: function(e){
                //console.log(e);
                SAlert.showError(e);
    ...
    function submit_create_department(){
        $("#submit_create_department").click(function(){
            ...
            $.ajax({
                ...
                error:function(e){
                    if(e.status==409){
                        display_check_info("#create_name_div", false, "该名称已存在，无法使用")
                        return;
                    }
                    SAlert.showError(e);
                }
    ...
    function submit_edit_department(){
        $("#submit_edit_department").click(function(){
            ...
            $.ajax({
                type:"PUT",
                url:project_url + "/departments/"+department_id,
                data: JSON.stringify(data),
                headers: {
                    'Content-Type': 'application/json',
                    "RC-Token": $.cookie("token_id")
                ...
                error:function(e){
                    if(e.status==409){
                        display_check_info("#edit_name_div", false, "该名称已存在，无法使用");
                        return
                    }
                    SAlert.showError(e);
                }
