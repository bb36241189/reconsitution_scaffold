app.controller('serverController',['$scope','SAlert','$q','ServerService','UtilService',function ($scope,SAlert,$q,ServerService,UtilService) {
    var server_interval = [];
...
function get_all_servers(search_name) {
    ServerService.getServersBySearchName(search_name).success(function (data) {
...
function init_server_detail(server_id) {
    ...
    ServerService.getServersById(server_id).success(function (data) {
...
function delete_servers() {
    ...
    for (var i = 0; i < $servers_td.length; i++) {
        ...
        ServerService.deleteServersById(server_id).success(function () {
            refresh_server_list()
        }).error(function (e) {
            SAlert.showError(e)
        });
...
function get_images() {
    ...
    ServerService.getImages().success(function (data) {
...
function get_flavors() {
    ...
    ServerService.getFlavors().success(function (data) {
        flavor_module(data);
...
function get_network() {
    ...
    ServerService.getNetworks().success(function (data) {
...
function create_server() {
    if (check_create_params()) {
        ...
        ServerService.postServers(request_data).success(function () {
...
function edit_server() {
    ...
    ServerService.putServersById(server_id,request_data).success(function () {
        console.log("Request send success!");
...
function migrate_server() {
    ...
    if (params['action'] == "live_migrate") {
        if (check_params_on_migration(params)) {
            ...
            ServerService.postActionServers(params).success(function () {
                console.log("Request send success!");
                refresh_server_list();
                complete();
            }).error(function (e) {
...
function render_migration_table(origin_host_name) {
    ServerService.getAvailabilityZones().success(function (data) {
...
function get_interface_detail(server_id) {
    ...
    ServerService.getPortsByDeviceId(server_id).success(function (data) {
...
function get_network_list() {
    ...
    ServerService.getNetworks().success(render_interface_network_list).error(function (e) {
        SAlert.showError(e)
    });
}
...
function create_interface(server_id, network_id) {
    ...
    ServerService.postInterfaceServersById({
        serverId : server_id,
        net_id : network_id
    }).success(function () {
        init_interface_modal();
...
function delete_interface(server_id, port_id) {
    ...
    ServerService.deleteInterfaceServersById({
        serverId : server_id,
        port_id : port_id
    }).success(function () {
...
function get_floatingips() {
    var network_id = get_ext_network_id().then(function (network_id) {
        if (network_id == null) {
...
function get_ext_network_id() {
    ...
    var path = project_url + '/connect-network/' + port_id;
    var data = undefined;
    var defer = $q.defer();
    ServerService.getConnectNetworkByPortId(port_id).success(function (_data) {
        next(_data);
    }).error(function (e) {
        SAlert.showError(e)
        defer.reject(e);
    ...
    function next(_data){
        ...
        defer.resolve(data.external_network_id);
    }
...
function render_server_ports(data) {
    devide_interface_by_floatingip(data).then(function (iface_map) {
...
function devide_interface_by_floatingip(port_data) {
    ...
    get_floatingip_by_fixip(port_data).then(function (port_data) {
        next(port_data);
    });

    function next(port_data){
...
function get_floatingip_by_fixip(port_data) {
    ...
    function startBuidParamEnum(){
        var fix_ip = [];
        for (var i = 0; i < port_data.length; i++) {
            fix_ip.push(port_data[i].ip_address)
        }
        stepGetFloatingips(fix_ip);
    }

    function stepGetFloatingips(fix_ip){
         var paramStr = UtilService.getEnumParams4GetAction('fixed_ip_address',fix_ip);
         ServerService.getFloatingips(paramStr).success(buidPortData).error(function (e) {
             if (e.error.code != 404) {
                SAlert.showError(e)
            }
         });
...
function get_ext_network() {
    var path = project_url + "/networks";
    get_ext_network_id().then(function (network_id) {
        if (network_id) {
            $("#external_network_container").show().removeAttr("disabled");
            var params = {
                "router:external": 1,
            ...
            ServerService.getNetworks(params).success(render_ext_net_list).error(function () {
                SAlert.showError(e)
            });
        } else {
            //$("#floatingip_warnning").html("该内部网络没有连接到外部网络，无法绑定");
...
function sent_bind_request(_params) {
    ...
    if (type == "directly_bind") {
        ...
        var path = project_url + "/floatingip/" + floatingip_id;
        var params = {
        ...
        ServerService.putFloatingipById(floatingip_id,params).success(function () {
        ...
        ServerService.postFloatingips(params).success(function () {
...
function get_snapshot_list() {
    ...
    ServerService.getServersSnapshotsByInstanceId(server_id).success(function (data) {
...
function get_snapshot_info(snpashot_id) {
    ...
    var path = project_url + "/servers/" + server_id + "/snapshot/" + snpashot_id

    ServerService.getServicesBySnapshotAndServerId({
        serverId : server_id,
        snapshotId : snpashot_id
    }).success(render_snapshot_info).error(function (e) {
        SAlert.showError(e);
    });
    ...
    $("#create_snapshot_submit").off("click").on("click", function () {
        if ($("#create_snapshot_submit").attr("action-type") == "link_to_create") {
            ...
            if (check_create_snapshot_params(request_data)) {
                ...
                ServerService.postServersSnapshotsByInstanceId(server_id,request_data).success(function () {
                    complete();
                }).error(function (e) {
                    SAlert.showError(e);
                    complete();
    ...
    $("#edit_snapshot_submit").off("click").on("click", function () {
        ...
        if (snapshot_list.length == 1) {
            ...
            if (check_create_snapshot_params(params)) {
                ...
                ServerService.putServersSnapshotsByInstanceId(snapshot_id,params).success(function () {
...
function register_delete_snapshot() {
    $("#delete_snapshot").off("click").on("click", function () {
        ...
        ServerService.deleteSnapshotBySnapshotIdWithServerId({
            serverId : server_id,
...
function register_roll_back_snapshot() {
    $("#rollback_snapshot").off("click").on("click", function () {
        ...
        var $servers_td = $("#server_table tbody tr input:checked");
        var server_id = $servers_td.closest('tr').attr('id');
        var path = project_url + "/servers/" + server_id + "/snapshot/" + snapshot_id;
        ...
        ServerService.postSnapshotBySnapsshotIdWithServerId({
            serverId : server_id,
            snapshotId : snapshot_id
        }).success(function () {
...
function getTags(_tr){
    ...
    ServerService.getTags().success(function (data) {
        if(_tr!=undefined){
...
function registerTagButton(){
    $("#submit_tag").on("click",function(){
        ...
        if(data.tag_list.length){
            ServerService.postTagBind(data).success(function () {
                if(undata.tag_list.length){
                    ServerService.deleteTagBind(JSON.stringify(undata)).success(function () {
                        location.reload();
                    }).error(function (e) {
...
function click_clone_server(){
    $("#clone").click(function(){
        ...
        ServerService.getFlavors().success(render_clone_flavor).error(function (e) {
             SAlert.showError(e);
        });

        ServerService.getNetworks().success(render_clone_network).error(function (e) {
            SAlert.showError(e);
        });

        var get_keypairs = function(){
            ServerService.getKeypairs().success(render_clone_keypair).error(function (e) {
                SAlert.showError(e)
            });
        };
        ...
        {
            ServerService.getAvailabilityZones().success(function (ret) {
                clone(ret.availabilityZoneInfo);
            }).error(function (e) {
                SAlert.showError(e);
            });
...
function submit_clone_server(){
    $("#submit_clone").click(function(){
        ...
        ServerService.postServerClone(server_id,data).success(function () {
            location.reload();
        }).error(function (e) {
...
function click_backup_server(){
    $("#backup").click(function(){
        ...
        var get_snapshots = function(){
            var path = project_url + "/servers/snapshots";
            var params = {
                "instance_id": server_id
            };
...
function submit_backup_server(){
    $("#submit_backup").click(function(){
        ...
        ServerService.postServerBackups(data).success(function () {
            location.reload();
        }).error(function (e) {
            $("#clone_modal").modal('hide');
            SAlert.showError(e);
...
function get_billPolicy(){
    ...
    ServerService.getActivePriceScheme().success(function (data) {
...
function get_keypair(){
    ServerService.getKeypairs().success(function (data) {
        renderServerKeypair(data);
    }).error(function (e) {
         SAlert.showError(e)
    });

    function renderServerKeypair(data){
...
function get_availability_zone(){
    ServerService.getAvailabilityZones().success(function (ret) {
        renderServerAvailabilityZone(ret.availabilityZoneInfo);
    }).error(function (e) {
        SAlert.showError(e)
    });

    function renderServerAvailabilityZone(){
